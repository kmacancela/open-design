...
describe('ChatComposer /search command', () => {
  ...
  it('shows the active imported-folder file and sends it as edit context', async () => {
    ...
    const input = screen.getByTestId('chat-composer-input') as HTMLTextAreaElement;
    expect(input.placeholder).toBe('Ask Open Design to change index.html...');
    fireEvent.change(input, { target: { value: 'Make the hero clearer' } });
    fireEvent.click(screen.getByTestId('chat-send'));
  ...
  it('sends the active project-location file as edit context', async () => {
    ...
    const input = screen.getByTestId('chat-composer-input') as HTMLTextAreaElement;
    fireEvent.change(input, { target: { value: 'Change the calendar background to red' } });
    fireEvent.click(screen.getByTestId('chat-send'));
